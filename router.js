...
class Router {
    ...
    this.routes = Object.entries(swagger.spec.paths).reduce((specs, _ref) => {
      ...
      return {
        ...
        parameters: {
          ...
          types: params.reduce((types, p) => _objectSpread(_objectSpread({}, types), {}, {
            [p.name]: p.type
          }), {})
        },
  ...
  updateState(state) {
    ...
    this.history.replace(_objectSpread(_objectSpread({}, location), {}, {
      state: _objectSpread(_objectSpread({}, location.state), state)
    }));
    this.isMuted = false;
  }
  ...
  route(to) {
    ...
    next.state = _objectSpread(_objectSpread({
      scroll: rewind ? referer.state.scroll : 0
    }, replace ? current.state : undefined), {}, {
      route,
      referer: pageChange && !rewrite ? _objectSpread(_objectSpread({}, current), {}, {
        state: _objectSpread(_objectSpread({}, current.state), {}, {
